package cn.gmlee.plugin.service.impl;

import cn.gmlee.plugin.service.TableInfoService;
import cn.gmlee.plugin.tool.*;
...
import cn.gmlee.plugin.entity.ColumnInfo;
import cn.gmlee.plugin.entity.SaveFile;
import cn.gmlee.plugin.entity.TableInfo;
import cn.gmlee.plugin.entity.TypeMapper;
import cn.gmlee.plugin.tool.*;
...
public class TableInfoServiceImpl implements TableInfoService {
    ...
    public void save(TableInfo tableInfo) {
        ...
        // 获取或创建保存目录
        VirtualFile dir = getJasCodeConfigDirectory(project);
        if (dir == null) {
            return;
        }
        // 获取保存文件
    ...
    private TableInfo read(TableInfo tableInfo) {
        // 获取保存的目录
        VirtualFile JasCodeConfigDir = getJasCodeConfigDirectory(project);
        if (JasCodeConfigDir == null) {
            return null;
        }
        // 获取配置文件
        String fileName = getConfigFileName(tableInfo.getObj());
        VirtualFile configJsonFile = JasCodeConfigDir.findChild(getConfigFileName(tableInfo.getObj()));
     ...
     *
     * @param project 项目
     * @return JasCodeConfig目录
     */
    private VirtualFile getJasCodeConfigDirectory(Project project) {
        ...
        // 查找或创建JasCodeConfig路径
        VirtualFile JasCodeDir = ideaDir.findChild("JasCodeConfig");
        if (JasCodeDir == null) {
            JasCodeDir = fileUtils.createChildDirectory(project, ideaDir, "JasCodeConfig");
        }
        return JasCodeDir;
    }
